...
class extends React.Component<Props, State> {
  ...
  sort = (a: Slot, b: Slot): number => {
    ...
    if (this.state.sortField === 'teacher') {
      return this.props.getUser(a.get('teacher')).get('displayname').localeCompare(
        this.props.getUser(b.get('teacher')).get('displayname'),
      ) * (this.state.sortUp ? 1 : -1);
    }
      ...
      <Grid container direction="column">
        ...
        <Grid item className={classes.table}>
          <Table>
            ...
            <TableBody>
              {this.props.slots.filter(this.filter).sort(this.sort).map(slot => (
                <Route
                  render={({ history }) => (
                    <TableRow
                      key={slot.get('_id')}
